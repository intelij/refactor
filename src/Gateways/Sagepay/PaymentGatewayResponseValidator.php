...
use Validator;
use DivideBuy\Payment\Exceptions\PaymentVerificationFailedException;
...
class PaymentGatewayResponseValidator
{
    ...
    {
        if ($this->gatewayResponse['Uid'] != config('payment.sagepay.uid')) {
    ...
    private function computeHash()
    {
        return strtoupper(
            sha1(
                ...
                config('payment.sagepay.clientSecret')
            )
        );
    }
}
